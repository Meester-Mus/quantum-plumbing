...
class PotentialEmbedding(nn.Module):
    ...
    def __init__(self, num_embeddings: int, embedding_dim: int, num_potentials: int = 8) -> None:
        super().__init__()
        self.num_embeddings = num_embeddings
        self.embedding_dim = embedding_dim
        self.num_potentials = num_potentials
        self.weight_potentials = nn.Parameter(
            torch.randn(num_potentials, num_embeddings, embedding_dim) / (embedding_dim**0.5)
        )

    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        H = torch.stack(
            [F.embedding(x, self.weight_potentials[i]) for i in range(self.num_potentials)],
            dim=0,
        )
